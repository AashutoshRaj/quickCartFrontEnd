...
import { useOrders, useDownloadInvoice } from '../queries/ordersQueries';
      ...
      <header className="sticky top-0 bg-background/80 backdrop-blur-md z-20 border-b border-outline/10">
        ...
        <div className="px-6 pb-4 space-y-3">
          ...
          {showFilter && (
            ...
            >
              {Object.entries(DATE_FILTERS).map(([key, config]) => (
                <button
                  key={key}
                  onClick={() => {
          ...
          >
            {filteredOrders.map((order, index) => (
              <OrderCard
                ...
                order={order}
                index={index}
                onDownload={() => handleDownloadInvoice(order._id || order.id)}
                onViewDetails={() => handleViewDetails(order._id || order.id)}
              />
            ))}
...
function OrderCard({ order, index, onDownload, onViewDetails }) {
                  ...
                  <div key={idx} className="flex items-center justify-between text-sm">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-secondary font-inter text-xs">
                        {item.quantity}x @ ${(item.price || item.unitPrice || 0).toFixed(2)}
                      </p>
                    </div>
            ...
            {order.storeAddress && (
              <div className="flex gap-2 p-3 bg-white rounded-lg border border-outline/10">
                ...
                <div>
                  ...
                  <p className="text-on-surface font-inter text-xs">{order.storeAddress}</p>
                </div>
              </div>
            )}
