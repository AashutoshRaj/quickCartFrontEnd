...
import { useScanStore } from '../hooks/stores/useScanStore';
import { ScannerOverlay } from '../components/ScannerOverlay';
import { setActiveStore } from '../store/slices/storeSlice';

export default function ScanStore() {
  ...
  const hasShownStoreRef = useRef(false);
  ...
  useEffect(() => {
    if (searchError) {
      if (searchError.response?.status === 404) {
        ...
        toast.error(searchError.response?.data?.message || 'Store is not available');
      } else {
        toast.error(searchError.message || 'Failed to load store');
      }
