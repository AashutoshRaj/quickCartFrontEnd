...
const defaultIntegrations: Integration[] = [
  { name: 'Stripe', desc: 'Payment processing', logo: '💳', status: 'Connected' },
  { name: 'Mailchimp', desc: 'Email marketing', logo: '📧', status: 'Not Connected' },
  { name: 'Google Analytics', desc: 'Traffic & conversion tracking', logo: '📊', status: 'Connected' },
  { name: 'Slack', desc: 'Order & alert notifications', logo: '💬', status: 'Not Connected' },
];
...
export function SettingsPage() {
  ...
  const handleTaxRateChange = (index: number, value: string) => {
    setTaxCategories(prev =>
      prev.map((t, i) => (i === index ? { ...t, rate: value } : t))
    );
    setTaxSaved(false);
  };
  ...
  const handleSaveTax = async () => {
    setTaxSaving(true);
    try {
      // TODO: replace with real API call, e.g. await updateTaxConfig(taxCategories);
  ...
  const handleSaveHours = async () => {
    setHoursSaving(true);
    try {
      // TODO: replace with real API call, e.g. await updateBusinessHours(hours);
      await new Promise(resolve => setTimeout(resolve, 500));
      setHoursSaved(true);
  ...
  // Handle integration connect/disconnect toggle
      ...
      <div className="flex gap-5">
        ...
        <div className="flex-1">
          ...
          {active === 'tax' && (
            <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm space-y-5">
              <div>
                <h3 className="text-gray-900 text-lg" style={{ fontWeight: 600 }}>Tax Configuration</h3>
                <p className="text-xs text-gray-500 mt-1">Set tax rates by product category</p>
              </div>
              <div className="space-y-3">
                {taxCategories.map((t, index) => (
                  <div key={t.category} className="flex items-center justify-between py-3 border-b border-gray-50 last:border-b-0">
                    <div>
                    ...
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        className="w-20 px-2 py-1.5 text-sm text-center border border-gray-200 rounded-lg outline-none focus:border-green-400 focus:ring-1 focus:ring-green-200 transition"
                      />
                    </div>
                  </div>
                ))}
              ...
              <button
                ...
                disabled={taxSaving}
                className={`px-6 py-2.5 text-white text-sm rounded-lg font-medium transition-all flex items-center gap-2 ${
                  taxSaving ? 'bg-gray-300 cursor-not-allowed' : 'bg-green-500 hover:bg-green-600 active:scale-95'
                }`}
              >
                {taxSaving && <Loader className="w-4 h-4 animate-spin" />}
                {taxSaved && !taxSaving && <Check className="w-4 h-4" />}
          ...
          {active === 'hours' && (
            <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm space-y-4">
              ...
              <div className="space-y-2">
                {hours.map((h, i) => (
                  <div key={h.day} className="flex items-center gap-4 py-2.5 border-b border-gray-50">
                    ...
                    <div
                      className={`relative w-10 h-5 rounded-full cursor-pointer transition-colors ${h.open ? 'bg-green-500' : 'bg-gray-200'}`}
                      onClick={() => setHours(hrs => hrs.map((x, j) => (j === i ? { ...x, open: !x.open } : x)))}
                    >
                      <div
                        className="absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform"
                        style={{ transform: h.open ? 'translateX(20px)' : 'translateX(2px)' }}
                      />
                    </div>
                    {h.open ? (
                      <div className="flex items-center gap-2">
                        <input
                          type="time"
                          value={h.from}
                          onChange={e => setHours(hrs => hrs.map((x, j) => (j === i ? { ...x, from: e.target.value } : x)))}
                          className="px-2 py-1 text-sm border border-gray-200 rounded-lg outline-none focus:border-green-400"
                        />
                        <span className="text-gray-400 text-sm">to</span>
                        <input
                          type="time"
                          value={h.to}
                          onChange={e => setHours(hrs => hrs.map((x, j) => (j === i ? { ...x, to: e.target.value } : x)))}
                          className="px-2 py-1 text-sm border border-gray-200 rounded-lg outline-none focus:border-green-400"
                        />
                      </div>
                    ) : (
              ...
              <button
                ...
                className={`px-6 py-2.5 text-white text-sm rounded-lg transition-colors flex items-center gap-2 ${
                  hoursSaving ? 'bg-gray-300 cursor-not-allowed' : 'bg-green-500 hover:bg-green-600'
                }`}
              >
                {hoursSaving && <Loader className="w-4 h-4 animate-spin" />}
                {hoursSaved && !hoursSaving && <Check className="w-4 h-4" />}
          ...
          {/* Coming Soon */}
          {!['store', 'payment', 'tax', 'hours', 'integrations', 'security', 'api', 'backup'].includes(active) && (
